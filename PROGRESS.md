...
Os **dois eixos já foram decididos** (2026-09-03): domínio novo aprovado, execução na máquina do
escritório 24h, PaaS descartada para a central, Cloudflare no plano gratuito. O passo a passo está em
...
**✅ Gate verificado ao vivo em 2026-09-03.** `bash scripts/verificar-operacao.sh` → **12 asserções verdes**. As provas que não cabem num script:
...
| Item | Risco | Alvo |
|---|---|---|
| **O `CENTRAL_ACCESS_TOKEN` do espelho pertence a uma PESSOA.** Medido em 2026-09-03: o token que o monorepo usa é do `adm@flashcapital.com.br` (administrator), não de uma conta de máquina — o oposto do que a `.claude/memory/security.md` afirmava. O seed já sabe criar a conta `espelho@flashcapital.com.br` e materializar o token do `.env`, mas **não troca sozinho**: `access_tokens.token` tem índice único, e falhar ali impediria web e sidekiq de subirem. | não dá para revogar o acesso da máquina sem derrubar o da pessoa; mexer no usuário do admin quebra o espelho **em silêncio** (AD-12) | rotação: gerar valor novo, pôr nos `.env` dos dois repos e rodar o seed. Barato — fazer junto com o próximo deploy |
