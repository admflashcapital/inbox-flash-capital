...
| # | Item | Repo | Esforço | Por que |
|---|---|---|---|---|
| **R1** | ✅ **FEITO em 2026-09-04** — a tarefa `WSL-Always-On` abre no logon uma janela `wsl.exe` que roda `sleep infinity` e nunca termina: é ela que segura a VM, porque o WSL encerra a instância 15 s depois que o último cliente do Windows sai, e nada dentro do Linux conta como cliente (A/B isolado em 2026-09-10). **A janela fica aberta**; fechada, ou depois de um `wsl --shutdown`, religa-se com `Start-ScheduledTask -TaskName "WSL-Always-On"`. Procedimento e reversão: **`docs/runbook-wsl-autostart.md`** | — | 15 min | é o elo que falta. Sem ele, "ligada 24h" é intenção, não disponibilidade |
...
> **Por que login automático, e não uma tarefa `-AtStartup`** — o login automático é a rota
> **provada**: 4 s do boot ao logon, a tarefa dispara no logon e a stack inteira volta em 19 s. A
> alternativa, uma tarefa "executar estando o usuário conectado ou não", roda na **sessão 0**, não
> interativa, e **não foi medida** aqui com o WSL. O relógio de 09/09 não a testa: o Windows subiu
> **02:42** e os containers só nasceram **12:32**, logo depois do logon das 12:23, porque a tarefa atual
> só dispara no logon. O Docker desta máquina é o Engine nativo, dentro da VM — não o Docker Desktop —,
> então qualquer rota que suba a VM sobe os containers junto.
